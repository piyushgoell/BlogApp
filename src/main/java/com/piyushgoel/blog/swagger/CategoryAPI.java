...
public interface CategoryAPI {
	...
	@PostMapping("/")
	default ResponseEntity<?> create(@Valid @RequestBody CategoryDTO categoryDTO) {
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
	@PutMapping("/{id}")
	default ResponseEntity<?> update(@PathVariable(name = "id") UUID Id, @Valid @RequestBody CategoryDTO categoryDTO ) {
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	}
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
	@DeleteMapping("/{id}")
	default ResponseEntity<?> delete(@PathVariable(name = "id") UUID Id) {
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	}
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
	@GetMapping("/{id}")
	default ResponseEntity<?> getCategoryById(@PathVariable(name = "id") UUID Id) {
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	}
	
	@Operation(summary = "Search Categories", description = "")
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
	@GetMapping("/")
	default ResponseEntity<?> getCategories(
			@Parameter(example = "abc") @RequestParam(value = "search" , required = false) String search,
